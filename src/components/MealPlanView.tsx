          ...
          <div className="space-y-1.5 max-w-lg">
            <div className="flex items-center gap-2.5">
              <span className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-500 border border-amber-500/20">
                ...
                {currentText.mealPlanProBadge || "AI"}
              </span>
              <h3 className="text-base sm:text-lg font-bold text-white font-['Outfit'] tracking-wide">
                {currentText.mealPlanAutoGeneratePro || "Generate weekly menu with AI"}
              </h3>
            </div>
            <p className="text-sm text-stone-400 leading-relaxed font-medium mt-1">
              {currentText.mealPlanAutoGenerateDesc || "AI uses your current pantry and culinary profile to propose a seven-day plan for review."}
            </p>
          </div>
          
          <button
            id="meal-plan-generate-ai-week"
            type="button"
            onClick={onGenerateAiWeekPlan}
            disabled={isGeneratingPlan || !onGenerateAiWeekPlan}
            className="w-full sm:w-auto px-5 py-3 rounded-xl font-bold text-sm tracking-wide transition-all shadow-[0_0_15px_rgba(245,158,11,0.2)] flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 disabled:cursor-not-allowed text-stone-950 border border-amber-400/50"
          >
            {isGeneratingPlan ? (
              <>
