...
export const ScanModal: React.FC<ScanModalProps> = ({
  ...
  const confirmationText =
    language === "es"
      ...
      : language === "bg"
      ? "Всяко попълнено поле е непотвърдено предложение за преглед от анализа на изображението с ИИ или справката по баркод. Името, категорията, цената и срокът на годност не са проверени данни; неизвестните стойности остават празни. Количеството и мерната единица трябва да бъдат потвърдени поотделно, преди да добавите продукта."
      : "Every populated field remains labelled as an unconfirmed, reviewable suggestion from AI image analysis or barcode lookup until you confirm it. Product name, category, cost, and expiry are not verified facts; unknown values stay blank. Quantity and unit must each be explicitly confirmed before the item can be added.";
