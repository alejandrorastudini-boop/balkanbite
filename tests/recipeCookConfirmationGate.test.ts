...
test("recipe card and cooking drawer stage intent instead of deducting directly", () => {
  assert.match(source, /const requestCookConfirmation = \(recipe: Recipe\) => \{\s*setCookFeedback\(null\);\s*setPendingCookRecipe\(recipe\);\s*\}/);
...
test("only explicit cook confirmation dispatches the inventory operation", () => {
  assert.match(source, /const confirmPendingCook = \(\) => \{\s*if \(!pendingCookRecipe\) return;\s*const outcome = handleCook\(pendingCookRecipe\)/);
